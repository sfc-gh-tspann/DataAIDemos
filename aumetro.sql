...
SELECT VEHICLELABEL, SPEED, STARTDATE, LATITUDE, LONGITUDE,
       HAVERSINE( LATITUDE, LONGITUDE, -34.9206, 138.5981 ) as distancefromdeloitteinkm,
       r.ROUTEID, i.ROUTEID, r.ROUTELONGNAME, r.ROUTEURL, r.ROUTEDESC, a.TRIPHEADSIGN, a.TRIPSHORTNAME, a.WHEELCHAIRACCESSIBLE
FROM DEMO.ADELAIDEVEHICLEPOSITIONS i  LEFT OUTER JOIN ADELAIDEROUTES r ON i.ROUTEID = r.ROUTEID
     LEFT OUTER JOIN ADELAIDETRIPS a ON a.TRIPID = i.TRIPID
where longitude is not null and latitude is not NULL
and LENGTH(TRIM(longitude)) > 0 and LENGTH(TRIM(latitude)) > 0
ORDER BY i.STARTDATE DESC,   i."TS" DESC;



SELECT i.ALERTTEXT , i.CAUSE , i.DESCRIPTIONTEXT , i.EFFECT ,
       r.ROUTEID, i.ROUTEID, r.ROUTELONGNAME, r.ROUTEURL, r.ROUTEDESC
FROM DEMO.ADELAIDESERVICEALERTS i  LEFT OUTER JOIN ADELAIDEROUTES r ON i.ROUTEID = r.ROUTEID
where i.ROUTEID  IS NOT null
ORDER BY  i."TS" DESC;
